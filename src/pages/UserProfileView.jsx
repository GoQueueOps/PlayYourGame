...
import { 
  ...
  ShieldAlert, Ban, Trophy, Zap, Ghost
} from "lucide-react";

function PlayerProfile({ userRole = "superadmin" }) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white pb-40 select-none font-sans italic">
      ...
      {/* --- IDENTITY SECTION --- */}
      <div className="p-6 bg-gradient-to-b from-[#0b0f1a] to-[#020617]">
        <div className="flex items-center gap-6 mb-8">
          <div className="relative">
             <div className="w-24 h-24 rounded-[2.5rem] bg-slate-800 overflow-hidden border-2 border-emerald-500/30 rotate-3 shadow-[0_0_20px_rgba(16,185,129,0.1)]">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${userData.name}`} alt="User" className="-rotate-3 scale-110" />
             </div>
             {/* AURA % INDICATOR */}
             <div className="absolute -bottom-2 -right-2 bg-emerald-500 text-black text-[10px] font-black px-3 py-1 rounded-lg border-2 border-[#020617] flex items-center gap-1">
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-[9px] font-black text-emerald-500/60 uppercase tracking-widest mt-2">Verified Athlete • {userData.location}</p>
            </div>
            {/* ADD FRIEND / RECRUIT BUTTON */}
            <button 
              onClick={() => setRequestSent(!requestSent)}
              className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${requestSent ? 'bg-white/5 text-slate-500 border border-white/10' : 'bg-white text-black shadow-lg hover:scale-105'}`}
            >
      ...
      <div className="px-6 mb-8">
        <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar">
          {['Badminton', 'Cricket'].map(sport => (
            <button 
              ...
              className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeSport === sport ? 'bg-emerald-500 text-black' : 'bg-white/5 text-slate-500'}`}
            >
              {sport}
            </button>
          ))}
        ...
        <div className="bg-[#0b0f1a] rounded-[2.5rem] p-8 border border-white/5 relative overflow-hidden shadow-2xl">
          ...
          <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-6">Peer Rank #455</p>
          <div className="grid grid-cols-2 gap-8">
            <div>
              <p className="text-[8px] font-black text-slate-600 uppercase mb-1 tracking-widest">Skill</p>
              <div className="text-xl font-black uppercase italic tracking-tighter text-white">Intermediate</div>
            </div>
            <div>
              <p className="text-[8px] font-black text-slate-600 uppercase mb-1 tracking-widest">Aura Class</p>
              <p className="text-xl font-black uppercase italic tracking-tighter text-emerald-400">Elite Alpha</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#020617] to-transparent pointer-events-none">
        <div className="bg-[#0b0f1a] border border-white/10 p-3 rounded-[2.5rem] shadow-2xl flex gap-3 pointer-events-auto">
          {/* MESSAGE BUTTON */}
          <button className="flex-1 bg-emerald-500 text-black rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-2 py-4 active:scale-95 transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)]">
            <MessageSquare size={18} fill="currentColor" /> Message
          </button>
          
          <div className="flex gap-2">
            {/* BLOCK BUTTON (Ghost icon for stealth) */}
