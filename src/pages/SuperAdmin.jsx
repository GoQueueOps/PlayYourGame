...
const INITIAL_ADMINS = [
  { id: 1, name: "Admin 1", phone: "9999900001", code: "AX101", role: "admin", status: "online" },
  { id: 2, name: "Admin 2", phone: "9999900002", code: "AX102", role: "admin", status: "online" },
];
...
const SuperAdmin = () => {
  ...
  const [newAdmin, setNewAdmin] = useState({ name: "", phone: "", code: "" });
  ...
  // ─── ADMIN MANAGEMENT ───
  const addAdmin = () => {
    if (newAdmin.name && newAdmin.phone && newAdmin.code) {
      setAdmins([...admins, { ...newAdmin, id: Date.now(), role: "admin", status: "offline" }]);
      setNewAdmin({ name: "", phone: "", code: "" });
    }
  };
  ...
  const NAV = [
    ...
    { id: "admin_mgmt", label: "Admin Management", icon: <UserPlus size={15} /> },
      ...
      <main className="flex-1 overflow-y-auto relative z-10">
        ...
        <div className="p-8 max-w-7xl mx-auto">
          ...
          {activeTab === "admin_mgmt" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-[#0b0f1a] border border-white/[0.06] p-6 rounded-2xl">
                <h3 className="text-xl font-black uppercase">Add New Admin</h3>
                <p className="text-[9px] font-bold text-slate-500 uppercase mt-1 mb-6">Create credentials for sub-admins</p>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">Admin Name</label>
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-black/40 border border-white/[0.07] rounded-xl pl-10 pr-4 py-3 text-[10px] font-bold outline-none focus:border-emerald-500/40"
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">Phone Number</label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">Secret Access Code</label>
                    <div className="relative">
                      <Key className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600" size={14} />
                      <input 
                        type="text" 
                        placeholder="Create Code (e.g. AX105)"
                        value={newAdmin.code}
                ...
                <motion.button
                  whileTap={{ scale: 0.98 }}
                  onClick={addAdmin}
                  className="w-full bg-emerald-500 hover:bg-emerald-400 text-black py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
                >
                  <UserPlus size={14} /> Authorize Admin Access
                </motion.button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {admins.map(admin => (
                  <motion.div 
                    layout
                    key={admin.id} 
                    className="bg-[#0b0f1a] border border-white/[0.06] p-5 rounded-2xl flex items-center justify-between group hover:border-emerald-500/20 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-slate-800 to-slate-900 border border-white/[0.05] flex items-center justify-center text-emerald-400">
                        <UserCheck size={20} />
                      </div>
                      <div>
                        <h4 className="font-black text-sm uppercase tracking-tight">{admin.name}</h4>
                        <div className="flex gap-3 mt-1">
                          <p className="text-[9px] font-bold text-slate-500 flex items-center gap-1"><Phone size={10} /> {admin.phone}</p>
                          <p className="text-[9px] font-bold text-orange-400 flex items-center gap-1"><Key size={10} /> {admin.code}</p>
                        </div>
                      </div>
                    </div>
                    <motion.button
