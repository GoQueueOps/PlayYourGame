...
import {
  ...
  MessageCircle, Send, UserPlus, Key, UserCheck,
  Star, Calendar, DollarSign, Loader2, ToggleLeft, ToggleRight
} from "lucide-react";
...
const SuperAdmin = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [arenasRes, playersRes, groupsRes, notifRes, bookingsRes, adminsRes] = await Promise.all([
        ...
        supabase.from('user_roles').select('user_id, roles(name), profiles(name, email)').in('roles.name', ['admin', 'superadmin'])
      ])
      ...
      if (adminsRes.data) setAdmins(adminsRes.data.filter(a => a.roles?.name))
  ...
  // ── Add admin by changing their role ──
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessages(prev => [...prev, {
      ...
      message: chatInput, created_at: new Date().toISOString(), role: 'super'
    }])
    setChatInput("")
  }
  ...
  const NAV = [
    ...
    { id: "approvals", label: "Owner Queue", icon: <Users size={15} /> },
    { id: "admin_mgmt", label: "Admin Management", icon: <UserPlus size={15} />, badge: admins.length },
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="bg-[#0b0f1a] border border-white/[0.06] p-6 rounded-2xl">
                  ...
                  <div className="space-y-3">
                    {[['Database', '● Online', 'text-green-400'], ['Payments', '● Razorpay', 'text-green-400'], ['Realtime', '● Active', 'text-green-400'], ['Admins Online', admins.length, 'text-purple-400']].map(([label, val, cls]) => (
          ...
          {activeTab === "admin_mgmt" && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-[#0b0f1a] border border-white/[0.06] p-6 rounded-2xl">
                ...
                <p className="text-[9px] font-bold text-slate-500 uppercase mt-1 mb-6">
                  Promote existing users to admin or superadmin role
                </p>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">User Email</label>
                    <div className="relative">
                      ...
                      <input type="email" placeholder="user@email.com"
                        ...
                        className="w-full bg-black/40 border border-white/[0.07] rounded-xl pl-10 pr-4 py-3 text-[10px] font-bold outline-none focus:border-purple-500/40" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest px-1">Assign Role</label>
                  ...
                  <div className="flex items-end">
                    <motion.button whileTap={{ scale: 0.98 }} onClick={handleAddAdmin} disabled={addingAdmin}
                      className="w-full bg-purple-500 hover:bg-purple-400 disabled:opacity-50 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-purple-500/20">
                ...
                <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-3 text-[9px] text-yellow-400/80 font-bold">
                  ⚠️ The user must already have an account. Their role will be updated immediately.
                </div>
              </div>

              {/* CURRENT ADMINS */}
              <div>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.3em] mb-4">Current Admins & Superadmins</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {admins.map((admin, idx) => (
                    <motion.div layout key={idx}
                      className="bg-[#0b0f1a] border border-white/[0.06] p-5 rounded-2xl flex items-center justify-between group hover:border-purple-500/20 transition-all">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${admin.roles?.name === 'superadmin' ? 'bg-purple-500/20 border-purple-500/30 text-purple-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
                  ...
                  {chatMessages.length === 0 ? (
                    ...
                    <div key={msg.id} className="flex justify-end gap-3">
                      <div className="max-w-xs">
                        <div className="px-4 py-2.5 rounded-lg text-[10px] bg-purple-500/15 border border-purple-500/20 text-slate-200">
                          {msg.message}
                        </div>
                      </div>
                    </div>
